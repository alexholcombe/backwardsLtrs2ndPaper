...
AA<- twoTarget %>% filter(subject=="AA")
#Function for one subject, calculating how many stream conditions failed the test
calcExclude<- function(oneSub) {
  oneSub %>% group_by(stream,orientation)
}
...
#B %>% print(n=Inf)
exclude <- B %>% filter(LRtestFails>=3, effFails > 
