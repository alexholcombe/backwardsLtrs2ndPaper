...
To be honest it is hard to know how to set the sigma (width of the prior in the above graph) because how much to think of the previous estimates as estimating the same thing versus being actually different effects?

To do the custom NEED STILL TO CHANGE THIS TO COHEN'S D?
...
Assume effect size is only `r smaller` as big as original `r meanCohensD`, yielding `r smaller`
...
#plot example small effect size points on curve
...
text(my.es+.1,valAlt+.1,paste0("BF=",round(valAlt/valNull,2)),col="red")
...
text(large.es+.3,valAlt+.1,paste0("BF=",round(valAlt/valNull,2)),col="red")
```
And for n=100
```{r subjexts100}
#For each effect size, compute density
