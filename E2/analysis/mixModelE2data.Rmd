...
```
Proportion of one-target trials seems to be `r table(is.nan(df$targetSP))["TRUE"] / sum( table(is.nan(df$targetSP)) )
`
...
Maybe do the one-target trials first, might be faster.
```{r, echo=FALSE}
one<- dg %>% filter(targetSide != "Both")

condtnVariableNames <- c("subject","targetSide", "stream", "orientation")  
#It seems there are no rows for one-target (targetSide!="Both") and stream!=targetSide,
# which would mean that I can't test for stream swaps in the one-target condition

estimates<- dg  %>%  #  filter(subject=="AA") %>%
