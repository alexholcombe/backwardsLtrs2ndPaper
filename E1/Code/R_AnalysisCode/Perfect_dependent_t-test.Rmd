...
alldata <- data.frame("Subject" =latency.wide$Subject, 
                  "LeftCanon" = latency.wide$LeftCanon, 
                  "RightCanon" = latency.wide$RightCanon)




#<- read.delim("demo_dependent.txt", header=TRUE, dec=".") #Define the name of the tab delimited txt file (or .csv file) that contains at least 3 columns: a subject identifier and 2 columns of the data in the two within-subject conditions. Specify the decimal symbol (e.g., your text file stores numbers either as 3.14 or 3,14)
...
xlabel<-"LeftCanon"  #name dependent variable 1 - needs to match the datafile!
ylabel<-"RightCanon"  #name dependent variable 2 - needs to match the datafile!

#Define name of individual subject identifier variable (e.g., the participantnumber)
subject<-"Subject"  #needs to match the datafile!
...
measurelabel<-"latency"

#Below names for axis are used. These CAN include spaces.
xlabelstring<-"stream" #define variables to be used for axis (can be replaced by "Any Label")
ylabelstring<-"latency"  #define variables to be used for axis
...
ci.sum <- ci.sum[order(ci.sum[[factorlabel]]),]
